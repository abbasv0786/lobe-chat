---
title: IntellectX Chat Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for IntellectX Chat Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest IntellectX Chat image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running IntellectX Chat Container

Assuming the IntellectX Chat container is named `lobe-chat`, use the following commands to stop and remove the currently running IntellectX Chat container:
...
### Pull the Latest IntellectX Chat Image

Use the following command to pull the latest Docker image for IntellectX Chat:
...
Redeploy the IntellectX Chat container using the newly pulled image:
...
No need to worry, you won't. All of IntellectX Chat's chat records are stored in your local browser. Therefore, when redeploying IntellectX Chat using Docker, your chat records will not be lost.
