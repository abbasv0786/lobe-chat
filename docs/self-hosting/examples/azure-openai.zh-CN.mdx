---
title: 在 IntellectX Chat 中集成 Azure OpenAI
description: 了解如何在 IntellectX Chat 中配置 Azure OpenAI。一步步指导从界面配置到部署时的环境变量设置。
tags:
  - Azure OpenAI
  - 配置指南
  - 使用限制
...
IntellectX Chat 支持使用 [Azure OpenAI](https://learn.microsoft.com/zh-cn/azure/ai-services/openai/concepts/models) 作为 OpenAI 的模型服务商，本文将介绍如何配置 Azure OpenAI。
...
| `ACCESS_CODE` | 可选 | 添加访问 IntellectX Chat 的密码，你可以设置一个长密码以防被爆破，该值用逗号分隔时为密码数组 | - | `awCT74` 或 `e3@09!` or `code1,code2,code3` |
