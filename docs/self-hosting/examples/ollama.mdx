---
title: Integrating IntellectX Chat with Ollama for Enhanced Language Models
description: >-
  Learn how to configure and deploy IntellectX Chat to leverage Ollama's powerful
  language models locally. Follow the guide to run Ollama and IntellectX Chat on your
  system.
tags:
  - Ollama integration
  - IntellectX Chat configuration
  - Local deployment
  - Language models
  - Ollama usage
---
...
Ollama is a powerful framework for running large language models (LLMs) locally, supporting various language models including Llama 2, Mistral, and more. Now, IntellectX Chat supports integration with Ollama, meaning you can easily use the language models provided by Ollama to enhance your application within IntellectX Chat.

This document will guide you on how to configure and deploy IntellectX Chat to use Ollama:
...
## Running IntellectX Chat Locally

Assuming you have already started the Ollama service locally on port `11434`. Run the following Docker command to start IntellectX Chat locally:

```bash
docker run -d -p 3210:3210 -e OLLAMA_PROXY_URL=http://host.docker.internal:11434 lobehub/lobe-chat
```

Now, you can use IntellectX Chat to converse with the local LLM.

For more information on using Ollama in IntellectX Chat, please refer to [Ollama Usage](/docs/usage/providers/ollama).
