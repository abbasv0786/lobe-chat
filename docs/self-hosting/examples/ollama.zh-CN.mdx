---
title: 在 IntellectX Chat 中集成 Ollama
description: 学习如何在 IntellectX Chat 中配置与部署 Ollama 语言模型框架，提升本地大模型使用体验。
tags:
  - Ollama
  - IntellectX Chat
  - 语言模型
...
本文档将指导你如何配置与部署 IntellectX Chat 来使用 Ollama：

## 本地启动 Ollama

首先，你需要安装 Ollama，安装与配置 Ollama 的详细步骤可以参考 [Ollama 官方站点](https://ollama.com)。

## 本地运行 IntellectX Chat

假设你已经在本地 `11434` 端口启动了 Ollama 服务。运行以下 Docker 命令行，在本地启动 IntellectX Chat：
...
关于在 IntellectX Chat 中使用 Ollama 的更多信息，请查阅 [Ollama 使用](/docs/usage/providers/ollama)。
