---
title: 在 Zeabur 上部署 IntellectX Chat
description: 按照指南准备 OpenAI API Key 并点击按钮进行部署。在部署完成后，即可开始使用 IntellectX Chat 并选择是否绑定自定义域名。
tags:
  - Zeabur
  - IntellectX Chat
  - OpenAI API Key
...
如果想在 Zeabur 上部署 IntellectX Chat，可以按照以下步骤进行操作：
...
# 使用 Zeabur 将 IntellectX Chat 部署为无服务器函数
...
由于 Zeabur 并未官方支持免费用户部署容器化服务，您可能希望将 IntellectX Chat 部署为无服务器函数服务。要在 Zeabur 上将 IntellectX Chat 部署为无服务器函数服务，您可以按照以下步骤操作：
...
### 将您的 IntellectX Chat 分支链接到刚创建的 Zeabur 服务。
