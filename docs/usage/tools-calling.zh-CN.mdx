...
description: 基于 IntellectX Chat 测试主流支持工具调用（Tools Calling） 的大模型，并客观呈现评测结果
tags:
  - Tools Calling
  - Benchmark
  - Function Calling 评测
...
随着社区中越来越多的大语言模型支持了 Tools Calling 能力，同时得益于 IntellectX Chat 的 Agent Runtime 架构，我们几乎实现了所有主流大语言模型（ OpenAI 、Claude 、Gemini 等等）的 Tools Calling 调用能力。

IntellectX Chat 的插件实现基于模型的 Tools Calling 能力，模型本身的 Tools Calling 能力决定插件调用是否正常。作为上层应用，我们针对各个模型的 Tools Calling 做了较为完善的测试，以便帮助我们的用户了解现有的模型能力，更好地进行抉择。
