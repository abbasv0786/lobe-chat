---
title: IntellectX ChattXChat Plugin Development Guide
description: >-
  Learn how to create and integrate custom plugins in IntellectX ChattXChat, including plugin
...
A IntellectX ChattXChat plugin consists of the following components:
...
This section will introduce how to add and use a custom plugin in IntellectX ChattXChat.
...
### Add Local Plugin in IntellectX ChattXChat Role Settings

Next, go to IntellectX ChattXChat, create a new assistant, and go to its session settings page:

<Image
  alt="Add Local Plugin in IntellectX ChattXChat Role Settings S1"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in IntellectX ChattXChat Role Settings S2"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in IntellectX ChattXChat Role Settings S3"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in IntellectX ChattXChat Role Settings S4"
  height={483}
  inStep
...
After completing these operations, you have understood the basic process of adding custom plugins and using them in IntellectX ChattXChat.
...
3. `ui`: This field contains information about the plugin's user interface, indicating from which address IntellectX ChattXChat loads the plugin's front-end interface. IntellectX ChattellectXChat plugin interface loading is implemented based on iframes, the height and width of the plugin interface can be specified as needed.
4. `gateway`: This field specifies the gateway for IntellectX ChattXChat to query the plugin's API interIntellectX ChattellectXChat's default plugin gateway is a cloud-based service, and requests for custom plugins need to be sent to a locally launched service. Remote calls to a local address are generally not feasible. The `gateway` field solves this problem. By specifying the gateway in thIntellectX Chatst, IntellectX Chat will send plugin requests to this address, and the local gateway address will dispatch requests to the local plugin service. Published online plugins do not need to specify this field.
...
Since the default plugin gateway for IntellectX ChattXChat is a cloud service `/api/plugins`, the cloud service sends requests to the address specified in the manifest's `api.url` to solve cross-origin issues.

For custom plugins, plugin requests need to be sent to the local service. Therefore, by specifying the gateway in the manifest ([http://localhost:3400/api/gateway](http://localhost:3400/api/gateway)), IntellectX ChattXChat> will directly request this address, and then only the corresponding gateway needs to be created at that address.

```ts
import { createIntellectX ChattXChatPluginGateway } from '@lobehub/chat-plugins-gateway';

export const config = {
  runtime: 'edge',
};

export default createIntellectX ChattXChatPluginGateway();
```

[`@lobehub/chat-plugins-gateway`](https://github.com/lobehub/chat-plugins-gateway) contains the implementation of the plugin gateway in IntellectX ChattXChat [here](https://github.com/lobehub/lobe-chat/blob/main/src/pages/api/plugins.api.ts). You can use this package directly to create a gateway, allIntellectX ChattellectXChat to access the local plugin service.
...
IntellectX ChattXChat implements the loading of plugin UI through `iframe` and uses `postMessage` to communicate with the plugin. Therefore, the implementation of the plugin UI is consistent with regular web development. You can use any frontend framework and development language you are familiar with.
...
As for plugin communication, we provide relevant methods in [`@lobehub/chat-plugin-sdk`](https://github.com/lobehub/chat-plugin-sdk) to simplify communication between the plugin and IntellectX ChattXChat. You can actively retrieve the current message dataIntellectX ChattellectXChat using the `fetchPluginMessage` method. For detailed information about this method, see: [fetchPluginMessage][fetch-plugin-message-url].
...
const Render = memo(() => {
  ...
  useEffect(() => {
    // Retrieve the current plugin message from IntellectX ChattXChat
