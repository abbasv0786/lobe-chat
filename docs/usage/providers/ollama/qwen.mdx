---
title: Using Local Qwen Model in IntellectX Chat
description: 通过IntellectX Chat和Ollama的集成，您可以轻松在IntellectX Chat中使用Qwen。本文将指导您如何在IntellectX Chat中使用本地部署版本的Qwen。
tags:
  - Qwen
  - IntellectX Chat
  - Ollama
...
<Image
  alt={'Using Qwen in IntellectX Chat'}
  cover
...
Now, through the integration of IntellectX Chat and [Ollama](https://ollama.com/), you can easily use Qwen in IntellectX Chat. This document will guide you on how to use the local deployment version of Qwen in IntellectX Chat:
...
In the IntellectX Chat conversation page, open the model selection panel, and then select the Qwen model.
...
/>
  ...
  <Callout type={'info'}>
    If you do not see the Ollama provider in the model selection panel, please refer to [Integration with Ollama](/docs/self-hosting/examples/ollama) to learn how to enable the Ollama provider in IntellectX Chat.

</Callout>
</Steps>

Next, you can have a conversation with the local Qwen model in IntellectX Chat.
