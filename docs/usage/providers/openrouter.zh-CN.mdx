---
title: 在 IntellectX Chat 中使用 OpenRouter API Key
description: 学习如何在 IntellectX Chat 中注册、创建 API Key、充值信用额度并配置 OpenRouter，以便开始使用多种优秀大语言模型 API。
tags:
  - OpenRouter
  - API Key
  - Web UI
...
# 在 IntellectX Chat 中使用 OpenRouter

<Image
  alt={'在 IntellectX Chat 中使用 OpenRouter'}
  cover
...
本文档将指导你如何在 IntellectX Chat 中使用 OpenRouter:
...
- 点击 `Create Key` 开始创建
- 在弹出对话框中为 API 密钥取一个名字，例如 "IntellectX Chat Key"
...
### 步骤四：在 IntellectX Chat 中配置 OpenRouter

- 访问 IntellectX Chat 的 `设置` 界面
- 在 `语言模型` 下找到 `OpenRouter` 的设置项
- 打开 OpenRouter 并填入获得的 API 密钥

<Image
  alt={'在 IntellectX Chat 中配置 OpenRouter'}
  height={518}
  inStep
...
至此你已经可以在 IntellectX Chat 中使用 OpenRouter 提供的模型进行对话了。
