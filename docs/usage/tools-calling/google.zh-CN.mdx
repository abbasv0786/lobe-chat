...
description: >-
  使用 IntellectX Chat 测试 Google Gemini 系列模型（Gemini 1.5 Pro / Gemini 1.5
...
Gemini 输出的 json 中，name 是错误的，因此 IntellectX Chat 无法识别到它调用了什么插件。（入参中，天气插件的 name 为 `realtime-weather____fetchCurrentWeather`，而 Gemini 返回的是 `weather____fetchCurrentWeather`）。
