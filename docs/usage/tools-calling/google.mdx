...
description: >-
  使用 IntellectX Chat 测试 Google Gemini 系列模型（Gemini 1.5 Pro / Gemini 1.5 Flash）
...
In the json output from Gemini, the name is incorrect, so IntellectX Chat cannot recognize which plugin it called. (In the input, the name of the weather plugin is `realtime-weather____fetchCurrentWeather`, while Gemini returns `weather____fetchCurrentWeather`).
