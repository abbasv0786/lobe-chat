...
describe('UserAvatar', () => {
  describe('enable Auth', () => {
    ...
    it('should show IntellectX Chat and default avatar when the user is not logged in and enable auth', () => {
      act(() => {
        useUserStore.setState({ enableAuth: () => true, isSignedIn: false, user: undefined });
      });

      render(<UserAvatar />);
      expect(screen.getByAltText('IntellectX Chat')).toBeInTheDocument();
      expect(screen.getByAltText('IntellectX Chat')).toHaveAttribute('src', DEFAULT_USER_AVATAR_URL);
    });
  });

  describe('disable Auth', () => {
    it('should show IntellectX Chat and default avatar when the user is not logged in and disabled auth', () => {
      enableAuth = false;
      act(() => {
